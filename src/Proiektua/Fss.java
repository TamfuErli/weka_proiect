...
public class Fss {
    ...
    public static Instances applyFSS(Instances inputArff, String outputArff, String dictionaryFile) throws Exception {
        // Cargar archivo ARFF

        Instances data = inputArff;



        if (data.classAttribute().isNumeric()) {
            NumericToNominal filter = new NumericToNominal();
            filter.setAttributeIndices("last"); // Solo la clase
            filter.setInputFormat(data);
            data = Filter.useFilter(data, filter);
        }

        // Configurar y aplicar filtro
        AttributeSelection filter = createAttributeFilter(1000);
        filter.setInputFormat(data);
        Instances filteredData = filter.useFilter(data, filter);
